 ...
 * Deprecated
 */
// TODO 2024-07-03 Highly recommand to remove this file, it's full of bugs
...
const NextChunkTest = ({
  ...
  const latestChunk =
    topicDetails?.details?.chapter_chunks[
      topicDetails?.details?.chapter_chunks.length - 1
    ];
  ...
  const handleNextChunk = async () => {
    ...
    const processDataChunk = async (dataChunk: string) => {
      try {
        let buffer = "";
    ...
    <div>
      <div className="w-1/2 flex gap-3 justify-between items-center">
        <Button
          ...
          className="flex gap-1 bg-[#1A1647] items-center hover:bg-background hover:text-white"
          variant="outline"
        >
          {regenerateLoading ? (
            <>
