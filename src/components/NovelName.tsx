...
export function SubmitButton() {
    ...
    return (
        <Button disabled={pending} type='submit' variant='outline' className="flex gap-2  hover:bg-background hover:text-white" >
            {
                pending ?
                    <>
        ...
        <div className="border-b text-white flex gap-4 px-2 items-center border-input pb-2 font-semibold capitalize text-[1.1rem]">
            {isEditing ? (
                <div className='w-1/2 '>
                    <form action={formAction} className='flex items-center gap-4 w-full'>
                        <div className='relative w-full'>
                            ...
                            />
                            <button type="button" onClick={() => setIsEditing(false)} className="text-white top-[7px] absolute right-[10px] hover:text-gray-300 p-1">
                ...
                <>
                    ...
                    <FaEdit onClick={() => setIsEditing(true)} className="text-sm cursor-pointer" />
                </>
            )}
        </div>
    );
