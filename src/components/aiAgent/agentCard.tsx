...
const AgentCard: React.FC<PropsWithChildren<IAgentCardProps>> = ({
  ...
  return (
    <div
      style={style}
      className={`${className} w-full flex flex-col bg-[#170F21] agent-card border-2 shadow-md border-input p-6 text-sm rounded-md`}
    >
      <div className="title flex">
        ...
        {activeTab > 0 && (
          ...
          >
            {editStatus == "edit" ? (
              <FaRegSave size={30} color={"#fff"} />
            ) : (
              <FaRegEdit size={30} color={"#fff"} />
            )}
          </div>
        )}
        {agent.name && (
